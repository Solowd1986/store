...
class RequestHandler extends DbConnect
{
    ...
    {
        // Если нам пришел такой индекс, значит мы уже отправили последние доступные элементы,
        // и дальше отдаем только пустой массив
        if ($index === "-1") {
            return [
                "lastIndex" => -1,
                "load" => []
        ...
        // Начальный индекс, на его основе формируется $last_index, то есть индекс последнего отправленного элемента.
        // Отправили 8 элементов, значит 8 - индекс последнего. Индекс тут это не индекс элементов массива, и именно количество
        // элементов, которые отправили, по-умолчанию отправка идет по 8. Первым от клиента приходит индекс 0, то есть 0 элементов
        // получено. Если это так, то устанавливаем количество элементов в 8, именно столько получит клиент после нашего ответа.
        // Потом забираем из базы 8 записей, это регулируется $cnt_Of_rows. Потом 3 аргументом отправляем $last_index
        // Для getCategoryItems это будет offset при запросе к базе. Почему offset используется сразу? Потому что первые 8 записей
        // уже показаны при первой загрузке страницы категорий.
        $base_index = 8;
        $cnt_Of_rows = 8;
