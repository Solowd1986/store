    ...
    render() {
        ...
        return (
            <div className={styles.order__img_wrapper}>
                <img width={480} height={480} ref={this.mainImgRef} className={styles.order__img} src={`${list[0]}`} alt={alt} />

                <div className={styles.slider} ref={this.listSmallImgRef}>
                    {list.map((path) => (
                        <img key={path} width="60" height="60" onClick={this.toggleSlide} className={styles.img} src={path} alt={alt} />
                    ))}
                </div>
            </div>
        );
