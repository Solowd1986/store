...
const validationSchemaFields = [
    {
        name: yup
            ...
            .matches(/^[а-яА-Я]+$/, "Имя должно состоять из кириллицы")
            .min(3, "Имя должно включать не менее 3 символов")
    ...
    },
    {
        login: yup
            .string()
            .required("Данное поле обязательно")
            .matches(/^[a-zA-Z]+$/, "Логин должен состоять из латиницы")
            .min(3, "Логин должен включать не менее 3 символов")
            .max(15, "Логин должен включать не более 15 символов"),
    },
    {
        email: yup
            .string()
            .required("Данное поле обязательно").email("Введите корректный email"),
    },
    {
        address: yup
            .string()
            .required("Данное поле обязательно")
            .matches(/^[а-яА-Яa-zA-Z0-9.:\-,\s]+$/, "Введите корректный адрес")
            .max(200, "Адрес должен включать не более 200 символов"),
    },
    {
        phone: yup
            .string()
            .required("Данное поле обязательно")
            .min(18, "Телефон должен включать не менее 11 символов")
