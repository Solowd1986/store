...
const SortPorductsHook = ({ sortType = "по популярности", changeSortTypeCallback }:SortProductsProps) => {
    const [isSortPanelShow, toggleSortPanelVisibility] = useState(false);
    const listOfSortTypes = useRef<HTMLUListElement>();


    const toggleSortPanel = (evt: React.MouseEvent) => {
        evt.stopPropagation();
        if (!isSortPanelShow && listOfSortTypes.current) {
    ...
    const changeSortType = (evt: React.MouseEvent<HTMLElement>) => {
        evt.stopPropagation();
        if (!(evt.target instanceof HTMLElement)) return;
        if (evt.target.nodeName === "LI" && evt.target.innerText !== sortType) {
            changeSortTypeCallback(evt.target.innerText);
    ...
    // ПРОВЕРЬ ОТПИСЫВАЕТСЯ ЛИ СЛУШАТЕЛЬ
    useEffect(() => {
    ...
    return (
        <>
            <div className={styles.sort_wrapper}>
                ...
                <ul className={classList} onClick={changeSortType} ref={listOfSortTypes}>
                    <li>по новинкам</li>
...
class SortPorducts extends PureComponent<SortProductsProps, SortProductsState> {
    ...
    changeSortType = (evt: React.MouseEvent<HTMLElement>) => {
        ...
        if (evt.target.nodeName === "LI" && evt.target.innerText !== this.props.sortType) {
            this.props.changeSortTypeCallback(evt.target.innerText);
